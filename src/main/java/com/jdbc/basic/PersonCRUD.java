...
public class PersonCRUD {
    ...
    public static boolean update(String name, String ssm) {
        // 1. SQL구문을 작성
    ...
    public static List<Person> findAll() {
        ...
        try (Connection conn = Connect.makeConnection()) {
            ...
            // 5. SQL 실행 - SELECT실행시 executeQuery() // select만 쓴다

            // ResultSet : 조회 결과 나타는 2차원의 표
            ResultSet rs = pstmt.executeQuery();

            // ResultSet 을 순회해서 소비
            // a - next() 메서드를 통해 행들을 순서대로 지목
            // b - getter 메서드를 통해 열데이터를 추출
            while (rs.next()) { // 처음부터 1줄씩 +돼면서 넘어감 지목할게없으면 종료
            ...
            return Collections.emptyList(); // 빈 리스트 반환 null 해도됌
        }
    }
